...
export const ApiKeysTable: React.FC<ApiKeysTableProps> = ({
    ...
    const columns = useMemo(
        () => getColumns({ t, onOpenRotate, onOpenDelete }),
        [t, onOpenRotate, onOpenDelete]
    )
    ...
    return (
        <div className="space-y-4">
                            ...
                            <TableRow>
                                <TableCell colSpan={columns.length} className="h-40 text-center">
                                    <div className="flex flex-col items-center justify-center gap-2 text-muted-foreground">
                                        <KeyRoundIcon className="size-8 stroke-1 text-muted-foreground/60" />
                                        <span className="text-sm font-medium">
                                            {t('pages.private.tenant.api_keys.table.empty_state')}
                                        </span>
                                    </div>
                                </TableCell>
                            </TableRow>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-1 text-xs text-muted-foreground">
                ...
                <div>
                    <Trans i18nKey="pages.private.tenant.api_keys.table.showing_info" values={{ showing: data.length, total }}
                        components={{
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span className="mr-2">
                        <Trans i18nKey="pages.private.tenant.api_keys.table.page_info" values={{ page, totalPages: totalPages || 1 }}
                            components={{
