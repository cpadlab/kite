...
export const OrderSelect: React.FC<OrderSelectProps> = ({ sortOrder, onSortOrderChange }) => {
        ...
        <DropdownMenu>
                ...
                render={
                    <Button variant="outline" size="sm" className="gap-2 cursor-pointer h-8">
                        ...
                        <span className="hidden sm:inline">
                            {sortOrder === 'desc'
                                ? t('pages.private.tenant.api_keys.table.sort_recent')
                                : t('pages.private.tenant.api_keys.table.sort_oldest')}
                        </span>
                    </Button>
                }
            />
            ...
            <DropdownMenuContent align="end" className="w-auto">
                <DropdownMenuItem className="gap-2 cursor-pointer" onClick={() => onSortOrderChange('desc')}>
                    ...
                    <span>{t('pages.private.tenant.api_keys.table.sort_recent_desc')}</span>
                </DropdownMenuItem>
                <DropdownMenuItem className="gap-2 cursor-pointer" onClick={() => onSortOrderChange('asc')}>
                    <ArrowUpAZIcon className="size-4" />
                    <span>{t('pages.private.tenant.api_keys.table.sort_oldest_asc')}</span>
                </DropdownMenuItem>
            </DropdownMenuContent>
